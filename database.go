...
// This updates both /users/id/{id} and /users/{username} locations
// Uses the same StorageUser structure as the auth service
func updatePasswordInAuthDB(userID, hashedPassword string) error {
	...
	// Parse the user data using the same structure as auth service (StorageUser)
	// StorageUser has: id, username, email, password
